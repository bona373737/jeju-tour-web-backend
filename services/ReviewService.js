...
class ReviewService{
    ...
    async selectListRef(params){
        ...
        try {
            //특정 memberno의 like데이터 조회
            dbcon = await DBPool.getConnection();
            let sql = mybatisMapper.getStatement('ReviewMapper','selectListRef',params)
            let [result] = await dbcon.query(sql);
    ...
    async selectCountRef(params){
        let dbcon = null;
        let cnt = 0;
        try {
            dbcon = await DBPool.getConnection();
            let sql = mybatisMapper.getStatement('ReviewMapper','selectCountRef',params)
            let [result] = await dbcon.query(sql);
